...
public class OrderService {
	...
	public OrderEnrichedDTO findOrderEnrichedById(Long id) {
		...
		if (order != null) {
			...
			try {
				...
				orderPaymentDetailDTO = new OrderPaymentDetailDto();
			}
			...
			orderEnrichedDTO
					.setOrderPaymentDetail(orderPaymentDetailDTO != null ? orderPaymentDetailDTO : new OrderPaymentDetailDto());
	...
	/**
	 * Método para guardar una orden, si no se pueden guardar todos los productos,
	 * se elimina la orden
	 * 
		...
		if (enrichedOrderDTO.getOrder() != null) {
			...
			if (orderEntity != null) {
				...
				for (ProductDTO productDTO : enrichedOrderDTO.getProducts()) {
					try {
						productClientFeign
								.createOrderProduct(new OrderProductDTO(orderEntity.getOrderId(), productDTO.getProductId()));
					} catch (Exception e) {
